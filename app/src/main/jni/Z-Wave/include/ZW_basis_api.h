...
extern BYTE                     /*RET TRUE if HW initialized */
 ApplicationInitHW(SW_WAKEUP bWakeupReason  /* Reason for the powerup of the chip */
    );
 ...
 */
extern void                     /*RET Nothing */
 ApplicationTestPoll(void);     /* IN Nothing */
...
extern void                     /*RET Nothing  */
 ApplicationCommandHandler(BYTE rxStatus,   /*IN  Frame header info */
...
extern BYTE                     /*RET Nothing */
 ApplicationAVNotify(BYTE bNotification, BYTE * bData);
...
BYTE                            /* RET TRUE if endpoint is valid, FALSE otherwise */
 ApplicationNodeInformationEx(BYTE endpoint,    /* IN endpoint to report on. Range: 1..N */
...
extern BYTE                     /*RET TRUE if operation was executed successfully, FALSE if not */
 ZW_SetRFReceiveMode(BYTE mode);    /* IN TRUE: On; FALSE: Off mode */
...
extern BYTE                     /*RET  FALSE if transmitter queue overflow   */
 ZW_SendNodeInformation(BYTE destNode,  /*IN  Destination Node ID  */
                        BYTE txOptions, /*IN  Transmit option flags         */
                        VOID_CALLBACKFUNC(completedFunc) (BYTE));   /*IN  Transmit completed call back function  */
...
extern BYTE                     /*RET Library type ZW_LIB_xxx  */
 ZW_Version(XBYTE * dst);       /* IN Destination pointer in RAM */
...
**
**--------------------------------------------------------------------------*/
extern void ZW_GetProtocolVersion(PROTOCOL_VERSION xdata * dst) /*IN Destination pointer in RAM */
;
...
ZW_SendTestFrame(BYTE nodeID,   /* IN nodeID to transmit to */
...
**    Set the trigger level for external interrupt 0 or 1
**---------------------------------------------------------------------------------------------*/
void ZW_SetExtIntLevel(BYTE intSrc, /*IN: The interrupt src to set its level ZW_INT0 or ZW_INT1 */
...
**--------------------------------------------------------------------------*/
void                            /*Nothing */
ZW_SetPromiscuousMode(BOOL state);  /* IN TRUE to enable the promiscuous mode, FALSE to disable it. */
...
**
**--------------------------------------------------------------------------*/
void ZW_IOS_enable(BYTE bState);    /* IN  byte containing IO state. TRUE: IO port states are controllable
