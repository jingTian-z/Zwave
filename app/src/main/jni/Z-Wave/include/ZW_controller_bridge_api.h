 ...
 * Last Changed:     $Date: 2013-11-19 10:19:07 +0100 (Tue, 19 Nov 2013) $
 *
 ****************************************************************************/
 ...
 *
 * \param[in] rxStatus IN Frame header info:
 ...
 * \param[in] destNode Command receiving Node ID.
 *                     Either Bridge Controller Node ID or virtual slave Node ID.
 *                     If received frame is a multicast frame then destNode is
 *                     not valid and multi points to a multicast structure
 *                     containing the destination nodes.
 ...
 *
 * \serialapi{ZW->HOST: REQ | 0xA8 | rxStatus | destNodeID | srcNodeID | cmdLength | pCmd[ ] | multiDestsOffset_NodeMaskLen | multiDestsNodeMask}
 *
 */
extern void                     /*RET Nothing  */
 ApplicationCommandHandler_Bridge(BYTE rxStatus,    /* IN  Frame header info */
...
**--------------------------------------------------------------------------*/
extern void                     /*RET Nothing  */
 ApplicationSlaveCommandHandler(BYTE rxStatus,  /*IN  Frame header info */
...
BYTE                            /*RET FALSE if transmitter busy or srcNodeID invalid or controller primary */
 ZW_SendSlaveData(BYTE srcNodeID,   /* IN Source node ID - Virtuel nodeID */
 ...
 */
extern void                     /*RET Nothing */
 ApplicationSlaveNodeInformation(BYTE destNode, /* IN Which node do we want the nodeinfo on */
...
BYTE                            /*RET FALSE if SlaveNodeinformation not in transmit queue */
 ZW_SendSlaveNodeInformation(BYTE sourceNode,   /* IN Which node is to transmit the nodeinfo */
...
BYTE                            /*RET Returns TRUE if successful or FALSE if node invalid or controller is primary */
 ZW_SetSlaveLearnMode(BYTE node,    /* IN nodeID on Virtual node to set in Learn Node Mode - if new node wanted then it must be ZERO */
...
**
**--------------------------------------------------------------------------*/
void ZW_GetVirtualNodes(BYTE * nodeMask);
