 ...
 * Revision:         $Revision: 29399 $
 * Last Changed:     $Date: 2014-08-08 15:25:36 +0200 (Fri, 08 Aug 2014) $
 *
 ****************************************************************************/
...
**    Function description
**      Returns TRUE if a cached routed is found.
**      Returns FALSE if no cached route is found.
...
**
**      Example: 0,0,0,0,ZW_LAST_WORKING_ROUTE_SPEED_100K ->
**                  Direct 100K
**               2,3,0,0,ZW_LAST_WORKING_ROUTE_SPEED_40K  ->
**                  40K route through repeaters 2 and 3
**
**    Side effects:
**
**--------------------------------------------------------------------------*/
#define ZW_GET_LAST_WORKING_ROUTE(bNodeID, pLastWorkingRoute) ZW_GetLastWorkingRoute(bNodeID, pLastWorkingRoute)

/*==========================   ZW_SetLastWorkingRoute   ======================
**    Function description
**      Returns TRUE if specified pLastWorkingRoute has been saved for bNodeID.
**      Returns FALSE if specified bNodeID is NOT valid and no LastWorkingRoute
**      has been saved for bNodeID.
**      pLastWorkingRoute MUST point to a ROUTECACHE_LINE_SIZE (5) byte sized
...
**               2,3,0,0,ZW_LAST_WORKING_ROUTE_SPEED_40K  ->
**                  40K route through repeaters 2 and 3
**               2,3,4,0,ZW_LAST_WORKING_ROUTE_SPEED_9600 ->
**                  9600 route through repeaters 2, 3 and 4
**
**    Side effects:
**
**--------------------------------------------------------------------------*/
#define ZW_SET_LAST_WORKING_ROUTE(bNodeID, pLastWorkingRoute) ZW_SetLastWorkingRoute(bNodeID, pLastWorkingRoute)
 ...
 *
 */
extern void ApplicationControllerUpdate(BYTE bStatus,   /*IN  Status of learn mode */
...
**
**--------------------------------------------------------------------------*/
extern void ZW_ReplicationReceiveComplete(void);
...
extern BYTE                     /*RET  FALSE if transmitter busy      */
 ZW_ReplicationSend(BYTE destNodeID,    /*IN  Destination node ID. Only single cast allowed */
...
extern void                     /*RET Nothing        */
 ZW_GetNodeProtocolInfo(BYTE bNodeID,   /* IN Node ID */
                        NODEINFO * nodeInfo);   /*OUT Node info buffer */
...
extern BOOL                     /*RET TRUE if assign was initiated. FALSE if not */
 ZW_AssignReturnRoute(BYTE bSrcNodeID,  /* IN Routing Slave Node ID */
                      BYTE bDstNodeID,  /* IN Route destination Node ID */
...
extern BOOL                     /*RET TRUE if delete return routes was initiated. FALSE if not */
 ZW_DeleteReturnRoute(BYTE nodeID,  /*IN Routing Slave */
                      VOID_CALLBACKFUNC(completedFunc) (BYTE bStatus)); /*IN  Transmit complete status */
...
**--------------------------------------------------------------------------*/
extern void                     /*RET Nothing */
 ZW_SetDefault(VOID_CALLBACKFUNC(completedFunc) (   /* IN Command completed call back function */
                                                    void));
...
BYTE                            /*RET true if node in failed node table, else false */
 ZW_isFailedNode(BYTE nodeID);  /* IN the failed node ID */
...
BYTE                            /*RET function return code */
 ZW_RemoveFailedNode(BYTE NodeID,   /* IN the failed nodeID */
...
BYTE                            /*RET return the result of the function call */
 ZW_ReplaceFailedNode(BYTE bNodeID, /* IN the nodeID on the failed node to replace */
...
BYTE                            /*RET TRUE target is a static controller */
 ...
 ZW_SetSUCNodeID(BYTE nodeID,   /* IN the node ID of the static controller to be a SUC */
...
**
**--------------------------------------------------------------------------*/
BYTE ZW_SendSUCID(BYTE node, BYTE txOption, VOID_CALLBACKFUNC(callfunc) (BYTE));
...
BYTE                            /* RET:  TRUE - SUC is known to this controller, FALSE - SUC unknown */
 ZW_RequestNetWorkUpdate(VOID_CALLBACKFUNC(completedFunc) (BYTE txStatus)); /* call back function indicates of the update sucessed or failed */
...
extern BOOL                     /*RET TRUE if process is started. FALSE if not */
 ZW_AssignSUCReturnRoute(BYTE bSrcNodeID,   /* IN Routing Slave Node ID */
                         VOID_CALLBACKFUNC(completedFunc) ( /* IN Callback function called when done */
                                                              BYTE bStatus));
...
extern BOOL                     /*RET TRUE if delete SUC return routes was initiated. */
                        /*    FALSE if a return route assign/delete is allready active */
 ZW_DeleteSUCReturnRoute(BYTE bNodeID,  /*IN Routing Slave Node ID */
...
BYTE                            /*RET TRUE neighbor discovery started */
 ZW_RequestNodeNeighborUpdate(BYTE bNodeID, /* IN Node id */
...
BYTE                            /*RET Number of neighbors */
 ZW_GetNeighborCount(BYTE bNodeID); /* IN Node ID on node to count neighbors on */
...
BOOL                            /*RET FALSE if transmitter busy */
 ZW_RequestNodeInfo(BYTE nodeID,    /*IN: node id of the node to request node info from it. */
...
**
**--------------------------------------------------------------------------*/
void ZW_SetLearnMode(           /*RET  Nothing        */
...
**
**--------------------------------------------------------------------------*/
void ZW_AddNodeToNetwork(BYTE bMode, VOID_CALLBACKFUNC(completedFunc) (LEARN_INFO *));
...
**
**--------------------------------------------------------------------------*/
void ZW_RemoveNodeFromNetwork(BYTE bMode, VOID_CALLBACKFUNC(completedFunc) (LEARN_INFO *));
...
**--------------------------------------------------------------------------*/
void
ZW_RemoveNodeIDFromNetwork(BYTE bMode,
                           BYTE bNodeID, VOID_CALLBACKFUNC(completedFunc) (LEARN_INFO *));
...
**
**--------------------------------------------------------------------------*/
void ZW_ControllerChange(BYTE bMode, VOID_CALLBACKFUNC(completedFunc) (LEARN_INFO *));
...
BOOL                            /*RET NONE ZERO if nodes are neighbours else ZERO */
 ZW_AreNodesNeighbours(BYTE bNodeA, /* IN first node id */
...
BYTE                            /*RET FALSE if write buffer full         */
 ZW_StoreHomeID(BYTE * homeID,  /* IN Home-ID            */
                BYTE nodeID);   /* IN Node-ID            */
